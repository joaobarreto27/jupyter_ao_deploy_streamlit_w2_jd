...
load_dotenv(dotenv_path=".env_prd")
...
def create_plot(df, plot_type):
    if plot_type == "bar":
        return go.Figure(data=[go.Bar(x=df["product_name"], y=df["price"])])
    elif plot_type == "line":
        return go.Figure(
            data=[go.Scatter(x=df.index, y=df["price"], mode="lines+markers")]
        )
    elif plot_type == "scatter":
        return go.Figure(
            data=[go.Scatter(x=df["product_name"], y=df["price"], mode="markers")]
        )
    elif plot_type == "pie":
        return go.Figure(data=[go.Pie(labels=df["product_name"], values=df["price"])])
...
def main():
    ...
    query = "SELECT DISTINCT product_name, price FROM products ORDER BY price DESC"
